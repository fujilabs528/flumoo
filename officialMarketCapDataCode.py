...
print("MARKET CAPITALIZATION MARKET REPORT - OCTOBER 20/2020")
###########################################################
variable = ["Alpha", "Bravo", "Charlie", "Delta", "Echo", "Foxtrot", "Golf", "Hotel", "India", "Juliet", "Kilo", "Lima",
            "Mike",
...
pussylistDF = pd.DataFrame()
pussylistDF["marketCap"] = pussylist

overlistDF = pd.DataFrame()
overlistDF["ticker name "] = overlist

mariners = (overlistDF.join(pussylistDF))

mariners['marketCap'] = pd.to_numeric(mariners['marketCap'],errors="coerce")


falcons= mariners.sort_values(by= "marketCap")
...
print("STATISTICS")
print("MAX", charlie["marketCap"])
print("MIN", delta["marketCap"])
print("MEAN", echo["marketCap"])
print("MEDIAN",foxtrot["marketCap"])
print("STAN DEV",golf["marketCap"])
print("VAR", hotel["marketCap"])
print("SKEW",india["marketCap"])
print("KURTOSIS",juliet["marketCap"])

# RANGE INFO
kilo= charlie["marketCap"]
kilo= float(kilo)
lima=delta["marketCap"]
lima=float(lima)
mike= (kilo-lima)
print("RANGE", mike)
november= mike/30
...
engine = create_engine('postgresql://postgres:password@localhost:5432/sp500xxx')
falcons.to_sql(auto_generating_file_title, engine)
