...
print("EPS MARKET REPORT - OCTOBER 15/2020")
###########################################################
...
alpha= pd.read_sql_table("pageOneScrape",engine)

###########################################################
# CONVERT OBJECT TO FLOAT64
alpha['eps'] = pd.to_numeric(alpha['eps'],errors="coerce")
# SORTED TABLE BY COLUMN
beta= alpha.sort_values(by= "eps")
#print(type(beta))
...
delta= alpha.min()
echo= alpha.mean()
foxtrot= alpha.median()
golf=alpha.std()
hotel= alpha.var()
india=alpha.skew()
juliet=alpha.kurtosis()

print("STATISTICS")
print("MAX", charlie["eps"])
print("MIN", delta["eps"])
print("MEAN", echo["eps"])
print("MEDIAN",foxtrot["eps"])
print("STAN DEV",golf["eps"])
print("VAR", hotel["eps"])
print("SKEW",india["eps"])
print("KURTOSIS",juliet["eps"])
...
#TRANSLATE PRIMARY KEY TO TICKER NAME VIA INDEX NUMBER
#print("TICKER NAME LIST")
for hhh in beta["index"]:
    #print(p[hhh])
    bigdadddy_list.append(p[hhh])

## INDIVIDUAL COLUMNS
#print("EPS VALUE LIST")
for h in beta["eps"]:
    #print(h)
    bigmomma_list.append(h)

#print(bigdadddy_list)
#print(bigmomma_list)

bigdadddy_listDF = pd.DataFrame()
bigdadddy_listDF["TICKER NAME LIST"] = bigdadddy_list

bigmomma_listDF = pd.DataFrame()
bigmomma_listDF["EPS VALUE LIST"] = bigmomma_list

mariners = (bigdadddy_listDF.join(bigmomma_listDF))
...
#print("LENGTH OF VALUES", len(values_list))
tony= plt.bar(index,values_list)
plt.show()
...
#print("DXC", p.index("DXC"))
#print("APA", p.index("APA"))
