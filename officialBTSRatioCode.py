...
alpha = pd.read_sql_table("investingCashFlow", engine)
tickerlist=[]
for x in alpha["tickername"]:
    tickerlist.append(x)
ohlist= []
for j in alpha["investingCashFlow"]:
    ohlist.append(j)
sexlist=[]
for r in p:
    y= tickerlist.index(r)
    v=ohlist[y]
    #print(v)
    sexlist.append(v)
#print(sexlist)
##########################################################
alpha = pd.read_sql_table("financingCashFlow", engine)
tickerlist1=[]
for x in alpha["tickername"]:
    tickerlist1.append(x)
ohlist1= []
for j in alpha["financingCashFlow"]:
    ohlist1.append(j)
sexlist1=[]
for r in p:
    y= tickerlist1.index(r)
    v=ohlist1[y]
    #print(v)
    sexlist1.append(v)
#print(sexlist1)
...
for d in range(505):
    try:
        n= sexlist[d] + sexlist1[d]
    except:
        n=0
    #print(n)
    shiplist.append(n)
#print(shiplist)
shiplistDF=pd.DataFrame()
shiplistDF["behindthescenesratio"]= shiplist
pDF=pd.DataFrame()
pDF["tickername"]=p
mariners= pDF.join(shiplistDF)
#print(mariners)
falcons = mariners.sort_values(by="behindthescenesratio")
print(falcons)
##########################################################
brotherlist=[]
for u in falcons["tickername"]:
    brotherlist.append(u)
brotherlistDF=pd.DataFrame()
brotherlistDF["tickername"]= brotherlist
sisterlist=[]
for k in falcons["behindthescenesratio"]:
    sisterlist.append(k)
sisterlistDF=pd.DataFrame()
sisterlistDF["behindthescenesratio"]=sisterlist
unclelist=[]
for bb in range(505):
    unclelist.append(bb)
unclelistDF=pd.DataFrame()
unclelistDF["rank"]=unclelist
patriots= unclelistDF.join(brotherlistDF).join(sisterlistDF)
print(patriots)
# YOU JUST HAVE TO BREAK DOWN FALCONS INTO LISTS,
# CREATE A RANK LIST, THEN PUT THEM TOGETHER
patriots.to_sql("behindthescenesratio",engine)