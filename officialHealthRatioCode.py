...
alpha = pd.read_sql_table("assets", engine)
tickerlist=[]
for x in alpha["tickername"]:
    tickerlist.append(x)
ohlist= []
for j in alpha["assets"]:
    ohlist.append(j)
sexlist=[]
for r in p:
...
#print(sexlist)
##########################################################
alpha = pd.read_sql_table("liabilities", engine)
tickerlist1=[]
for x in alpha["tickername"]:
    tickerlist1.append(x)
ohlist1= []
for j in alpha["liabilities"]:
    ohlist1.append(j)
sexlist1=[]
for r in p:
...
shiplistDF["healthratio"]= shiplist
pDF=pd.DataFrame()
pDF["tickername"]=p
mariners= pDF.join(shiplistDF)
#print(mariners)
falcons = mariners.sort_values(by="healthratio")
print(falcons)
##########################################################
brotherlist=[]
...
sisterlist=[]
for k in falcons["healthratio"]:
    sisterlist.append(k)
sisterlistDF=pd.DataFrame()
sisterlistDF["healthratio"]=sisterlist
unclelist=[]
for bb in range(505):
    unclelist.append(bb)
...
patriots.to_sql("healthratio",engine)