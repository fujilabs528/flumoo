...
for www in p:
    url = "https://finance.yahoo.com/quote/" + www + "/balance-sheet?p=" + www
    page = requests.get(url)
    soup = BeautifulSoup(page.content, "html.parser")
    my_table = soup.find("div", {"class": "D(tbrg)"})

    try:
        data = my_table.findAll("div", {"Ta(c) Py(6px) Bxz(bb) BdB Bdc($seperatorColor) Miw(120px) Miw(140px)--pnclg D(tbc)"})
    ...
    try:
        assets_list.append(xxx_list[0])
    except Exception as e:
        assets_list.append("0")
        f = " |  富士 • F U J I   L A B S • 実験室  | "
        print(f, e)
    try:
        liabilities_list.append(xxx_list[2])
    except Exception as e:
        liabilities_list.append("0")
        f = " |  富士 • F U J I   L A B S • 実験室  | "
        print(f, e)
    try:
        equity_list.append(xxx_list[4])
    except Exception as e :
        equity_list.append("0")
        f = " |  富士 • F U J I   L A B S • 実験室  | "
        print(f, e)
    try:
        totalcap_list.append(xxx_list[6])
    except Exception as e:
        totalcap_list.append("0")
        f = " |  富士 • F U J I   L A B S • 実験室  | "
        print(f, e)
...
print(len(liabilities_list))
print(len(equity_list))
print(len(totalcap_list))


assetsDF = pd.DataFrame()
assetsDF["assets"] = assets_list

liabilities_listDF = pd.DataFrame()
liabilities_listDF["liabilities"] = liabilities_list

equityDF = pd.DataFrame()
equityDF["equity"] = equity_list

totalcapDF = pd.DataFrame()
totalcapDF["totalcap"] = totalcap_list


mariners = (assetsDF.join(liabilities_listDF).join(equityDF).
            join(totalcapDF))
