...
shiplistDF["dominanceratio"]= shiplist
pDF=pd.DataFrame()
pDF["tickername"]=p
mariners= pDF.join(shiplistDF)
#print(mariners)
falcons = mariners.sort_values(by="dominanceratio")
print(falcons)
##########################################################
brotherlist=[]
...
sisterlist=[]
for k in falcons["dominanceratio"]:
    sisterlist.append(k)
sisterlistDF=pd.DataFrame()
sisterlistDF["dominanceratio"]=sisterlist
unclelist=[]
for bb in range(505):
    unclelist.append(bb)
...
patriots.to_sql("dominanceratio",engine)